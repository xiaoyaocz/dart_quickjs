...
class VisualStudioResolver implements ToolResolver {
  ...
  List<ToolInstance> parseVswhere(String vswhereStdout, [Logger? logger]) {
    ...
    // "description": "鏋勫缓鏂板紡搴旂敤鎵€闇€鐨勪竴鍒囨潯浠?- 鍏嶈垂鎻愪緵锛岀敤浜庨珮鏁堝伐浣溿€?,
    // 当VS环境为中文时，编码存在问题，先临时使用以下方法解决
    // 正则移除description中的乱码，避免json解析失败
    vswhereStdout = vswhereStdout.replaceAll(
      RegExp(r'"description":\s*".*?[^\x00-\x7F].*?",', dotAll: true),
      '"description": "",',
    );
